...
def solve_hackrx_puzzle() -> ProcessResponse:
    ...
    landmark_map = {
        ...
        "Seoul": "Times Square", "Cape Town": "Acropolis", "Istanbul": "Big Ben",
    ...
    flight_api_urls = {
        "Gateway of India": "[https://register.hackrx.in/teams/public/flights/getFirstCityFlightNumber](https://register.hackrx.in/teams/public/flights/getFirstCityFlightNumber)",
        "Taj Mahal": "[https://register.hackrx.in/teams/public/flights/getSecondCityFlightNumber](https://register.hackrx.in/teams/public/flights/getSecondCityFlightNumber)",
        "Eiffel Tower": "[https://register.hackrx.in/teams/public/flights/getThirdCityFlightNumber](https://register.hackrx.in/teams/public/flights/getThirdCityFlightNumber)",
        "Big Ben": "[https://register.hackrx.in/teams/public/flights/getFourthCityFlightNumber](https://register.hackrx.in/teams/public/flights/getFourthCityFlightNumber)",
        "default": "[https://register.hackrx.in/teams/public/flights/getFifthCityFlightNumber](https://register.hackrx.in/teams/public/flights/getFifthCityFlightNumber)"
    }
    
    try:
        # Step 1: Get City
        city_api_url = "[https://register.hackrx.in/submissions/myFavouriteCity](https://register.hackrx.in/submissions/myFavouriteCity)"
        logger.info(f"[Puzzle Step 1] Getting city from {city_api_url}")
        city_response = requests.get(city_api_url)
        city_response.raise_for_status()
        city_name = city_response.json().get("city")
        logger.info(f"✅ City received: {city_name}")

        # Step 2: Decode Landmark
        landmark = landmark_map.get(city_name)
        if not landmark:
            raise ValueError(f"Landmark for city '{city_name}' not found in map.")
        logger.info(f"✅ Landmark decoded: {landmark}")

        # Step 3: Choose Flight Path
        flight_url = flight_api_urls.get(landmark, flight_api_urls["default"])
        logger.info(f"✅ Flight path selected: {flight_url}")

        # Step 4: Get Flight Number
        flight_response = requests.get(flight_url)
        flight_response.raise_for_status()
        flight_number = flight_response.json().get("flightNumber", flight_response.text)
        logger.info(f"🎉 Puzzle solved! Flight number: {flight_number}")
        
        return ProcessResponse(answers=[str(flight_number)])

    except Exception as e:
        logger.error(f"💥 Puzzle solver failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Puzzle solver failed: {e}")
...
async def process_documents(
    ...
    # --- HYBRID LOGIC: Check for the special puzzle PDF ---
    if "FinalRound4SubmissionPDF.pdf" in url:
